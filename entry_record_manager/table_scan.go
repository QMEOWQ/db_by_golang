...
type TableScan struct {
	...
	e_rm_interface EntryRecordManagerInterface
	file_name      string
	current_slot   int
}

func NewTableScan(ts *ts.Transaction, table_name string, layout LayoutInterface) *TableScan {
...
func (tbl_s *TableScan) GetVal(field_name string) *Constant {
	if tbl_s.layout.Schema().Type(field_name) == INTEGER {
		return NewConstantInt(tbl_s.GetInt(field_name))
	}

	return NewConstantString(tbl_s.GetString(field_name))
}

func (tbl_s *TableScan) SetVal(field_name string, val *Constant) {
	if tbl_s.layout.Schema().Type(field_name) == INTEGER {
		tbl_s.SetInt(field_name, val.IVal)
	} else {
		tbl_s.SetString(field_name, val.SVal)
	}
}
...
func (tbl_s *TableScan) MoveToNewBlock() {
	...
	tbl_s.e_rm_interface = NewEntryRecordPage(tbl_s.ts, blk, tbl_s.layout)
...
func (tbl_s *TableScan) GetRID() RIDInterface {
	return NewRID(int(tbl_s.e_rm_interface.Block().Number()), tbl_s.current_slot)
}

func (tbl_s *TableScan) MoveToRid(r RIDInterface) {
	tbl_s.Close()
