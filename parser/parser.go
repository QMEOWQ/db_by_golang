...
/*
4. insert 语句解析
	INSERT_COMMAND -> INSERT INTO ID LEFT_BRACE FIELD_LIST 
	RIGHT_BRACE VALUES CONSTANT_LIST
...
func (sp *SqlParser) fieldList() []string {
	...
	if sp.isMatchTag(lexer.COMMA) {
		fields := sp.fieldList()
		field_list = append(field_list, fields...)
	}
...
func (sp *SqlParser) constList() []*query.Constant {
	const_list := make([]*query.Constant, 0)
	const_list = append(const_list, sp.Constant())

	if sp.isMatchTag(lexer.COMMA) {
		consts := sp.constList()
		const_list = append(const_list, consts...)
	}
...
func (sp *SqlParser) Insert() interface{} {
	sp.checkWordTag(lexer.INSERT)
	sp.checkWordTag(lexer.INTO)
	sp.checkWordTag(lexer.ID)
	tbl_name := sp.sqlLexer.Lexeme
	...
	sp.checkWordTag(lexer.VALUES)
	sp.checkWordTag(lexer.LEFT_BRACKET)
	vals := sp.constList()
	sp.checkWordTag(lexer.RIGHT_BRACKET)

	//return NewInsertData(tbl_name, flds, vals)

	insert_data := NewInsertData(tbl_name, flds, vals)
	insert_data_def := fmt.Sprintf("insert def : %s", insert_data.ToString())
	fmt.Println(insert_data_def)
...
func (sp *SqlParser) Delete() interface{} {
	sp.checkWordTag(lexer.DELETE)
	sp.checkWordTag(lexer.FROM)
	sp.checkWordTag(lexer.ID)
	tbl_name := sp.sqlLexer.Lexeme

	pred := query.NewPredicate()
	if sp.isMatchTag(lexer.WHERE) {
		pred = sp.Predicate()
	}

	//return NewDeleteData(tbl_name, pred)

	delete_data := NewDeleteData(tbl_name, pred)
	delete_data_def := fmt.Sprintf("delete def : %s", delete_data.ToString())
	fmt.Println(delete_data_def)
...
/*
6. update 语句解析
	MODIFY_COMMAND -> UPDATE ID SET FIELD Assign_OPERATOR EXPRESSION (WHERE PREDICATE)?
*/

func (sp *SqlParser) Modify() interface{} {
	sp.checkWordTag(lexer.UPDATE)
	sp.checkWordTag(lexer.ID)
	tbl_name := sp.sqlLexer.Lexeme

	sp.checkWordTag(lexer.SET)
	_, fld_name := sp.Field()

	sp.checkWordTag(lexer.ASSIGN_OPERATOR)
	new_val := sp.Expression()
	...
	//return NewModifyData(tbl_name, fld_name, new_val, pred)

	modify_data := NewModifyData(tbl_name, fld_name, new_val, pred)
	modify_data_def := fmt.Sprintf("modify def : %s", modify_data.ToString())
	fmt.Println(modify_data_def)
