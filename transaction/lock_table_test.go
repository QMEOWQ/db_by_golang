...
func TestGetManySLocksOneTime(t *testing.T) {
	lock_table := NewLockTable()
	blk := fm.NewBlockID("test_getslock", 1)

	lock_table.SLock(blk)
	err := lock_table.SLock(blk)

	require.Equal(t, err, lock_table.SLock(blk))
}

func TestGetXLockFailForOnlyUnlockOneSLockForTwoSLock(t *testing.T) {
	lock_table := NewLockTable()
	blk := fm.NewBlockID("test_getslock", 1)
...
func TestRoutinesWithSLockTimeout(t *testing.T) {
	...
	blk := fm.NewBlockID("test_slock_timeout", 1)
	lock_table := NewLockTable()
	lock_table.XLock(blk)
	...
	for i := 0; i < 3; i++ {
		require.Equal(t, err_array[i], errors.New("SLock expection, but XLock on this blk"))
	}
}
...
func TestRoutinesWithSLockAfterXLockRelease(t *testing.T) {
	var err_array []error
	var err_array_lock sync.Mutex
	blk := fm.NewBlockID("test_slock_afterxlockrelease", 1)
	lock_table := NewLockTable()
	lock_table.XLock(blk)
	var wg sync.WaitGroup
