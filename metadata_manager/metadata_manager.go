...
func (mdm *MetaDataManager) CreateIndex(idx_name string, tbl_name string, fld_name string, ts *ts.Transaction) {
	mdm.IndexManager.CreateIndex(idx_name, tbl_name, fld_name, ts)
}
...
func (mdm *MetaDataManager) GetIndexInfo(tbl_name string, ts *ts.Transaction) map[string]*IndexInfo {
	return mdm.IndexManager.GetIndexInfo(tbl_name, ts)
}
